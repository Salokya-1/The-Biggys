...
| **Attendance capture** | Class lists and the timetable are the foundation it needs, but attendance is Problem Statement 3; marking a register per class is a day's work on top of what exists |
...
| **Model-assisted reason review** | The rule-based checker is deliberate: offline, explainable, no key. A model could judge whether a reason is *plausible* for the person and date, which rules cannot — but it must never be the thing that refuses a request |
| **Drag a whole week or a repeating class** | A move edits one occurrence plus its weekly slot; moving a block of classes at once, or splitting a series from a date, needs a recurrence model rather than a pattern plus overrides |
| **Room layout templates and import** | Layouts are drawn per room; a library of common shapes and an import from the estates floor plan would save RTE an afternoon |
| **Capability change history in the UI** | Every permission change is in the audit log, but there is no per-user "who gave them this and when" view yet |
| **Two-person rule for publishing** | Publishing is one admin action with a full audit trail; some institutions require a second approver, which the state machine could express |
