...
import { findClashes, generateTimetable, overlaps, slotDate } from '../lib/timetable';
...
export async function timetableRoutes(app: FastifyInstance) {
  ...
  app.post('/timetable/generate', { preHandler: [allow('seating.generate')] }, async (req) => {
    ...
    const result = generateTimetable(
      semester.intake.sections.map((s) => ({ id: s.id, name: s.name, size: s._count.students })),
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await audit(tx, { ...actorOf(req), action: 'timetable.generate', entityType: 'Semester', entityId: semesterId, after: { slots: result.slots.length, unplaced: result.unplaced.length, sessionsPerWeek } });
    });
    return { slots: result.slots.length, unplaced: result.unplaced, sections: semester.intake.sections.length, offerings: semester.offerings.length };
  });
  ...
  async function assertNoClash(data: { semesterId: string; sectionId: string; teacherId: string; venueId: string | null; dayOfWeek: number; startTime: string; endTime: string }, excludeId?: string) {
    if (!overlaps(data.startTime, data.endTime, data.startTime, data.endTime)) throw badRequest('endTime must be after startTime');
    ...
    if (clashes.length) {
      ...
      throw conflict('Timetable clash', { clashes: describe });
    }
  }

  app.post('/timetable/slots', { preHandler: [allow('seating.generate')] }, async (req, reply) => {
  ...
  app.patch('/timetable/slots/:id', { preHandler: [allow('seating.generate')] }, async (req) => {
  ...
  app.delete('/timetable/slots/:id', { preHandler: [allow('seating.generate')] }, async (req) => {
    const { id } = parse(idParam, req.params);
    const before = await prisma.timetableSlot.findUnique({ where: { id } });
    if (!before) throw notFound('Slot not found');
  ...
  app.post('/timetable/slots/:id/exceptions', { preHandler: [allow('seating.generate')] }, async (req, reply) => {
    const { id } = parse(idParam, req.params);
  ...
  app.delete('/timetable/exceptions/:id', { preHandler: [allow('seating.generate')] }, async (req) => {
    const { id } = parse(idParam, req.params);
